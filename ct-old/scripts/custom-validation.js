...
//From custom.js -- for reference 
	...
	$('#register-btn').live("click", function() {
		...
		$.ajax({
            type : "POST",
            url  : "http://220.227.92.105/speedsell/api/register.php",
	...
	$('#login-btn').live("click", function() {
		...
		$.ajax({
            type : "POST",
            url  : "http://220.227.92.105/speedsell/api/login.php",
	...
	$(".scheme-types li").live("tap", function() {
		alert("hi");
		selected_type = $(this).find("[name='leftpanel-radio']").attr("leftid");
		getfilteredscheme();
	});

	$(".filter-types li").live("click", function() {
		$(this).parent().find("input[type='checkbox']:first").attr("checked",false).checkboxradio("refresh");
	});

	$(".filter-types li.ui-first-child").live("click", function() {
		$(this).parent().find("input[type='checkbox']").attr("checked",false).checkboxradio("refresh");
	});
	...
	function updateSchemelist() {
    	...
    	$.ajax({
            type : "GET",
            url  : "http://220.227.92.105/speedsell/api/scheme.php",
        	...
        	$.each(response.leftPanel,function(key, topics){
        		...
        		$.each(topics,function(topic,contents) {
        			...
        			$.each(contents,function(index,contentname){
        				...
        				li += '<li><input type="radio" name="leftpanel-radio" leftid="'+contentname+'" id="radio-'+contentname+'" value="on" /><label for="radio-'+contentname+'">'+str+'</label></li>';
        				//console.log(li);
        			});
